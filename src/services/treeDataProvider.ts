...
import { IScopeService, ScopeService } from './scope.service'
...
export const createTreeDataProvider = (
    scopeService: IScopeService,
    treeViewEventEmitter: EventEmitter<ScopeTreeItem | ScopeFileTreeItem | undefined>
): TreeDataProvider<ScopeTreeItem | ScopeFileTreeItem> => ({
    ...
    getChildren: (
        element?: ScopeTreeItem | ScopeFileTreeItem
    ): ProviderResult<(ScopeTreeItem | ScopeFileTreeItem)[]> => {
        // if (!element) {
        //     const scopes = scopeService.getScopes()
